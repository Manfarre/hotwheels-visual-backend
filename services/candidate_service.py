...
def score_candidate(evidence: Dict[str, Any], phrase: str) -> float:
    clean_tokens = [normalize_text(t) for t in evidence.get("cleanTokens", [])]
    ...
    if any(phrase_n in line for line in model_lines):
        score += 4.0
...
def build_candidates(evidence: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    for name in secondary:
        candidates.append(
            {
                "name": name,
                "score": score_candidate(evidence, name),
                "reason": "secondary_inference",
